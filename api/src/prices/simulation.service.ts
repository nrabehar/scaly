...
export const INSTRUMENTS = {
  'XAU/USD': { name: 'Gold', basePrice: 5100, volatility: 0.0008, decimals: 2 },
  'BTC/USD': {
    name: 'Bitcoin',
    basePrice: 96500,
    volatility: 0.002,
...
export class SimulationService {
  ...
  constructor() {
    for (const s of Object.keys(INSTRUMENTS)) {
      ...
      this.state[s] = { lastKnownPrice: cfg.basePrice, simulatedHistory: [] };
  ...
  private generateSimulatedCandle(
    ...
    const high = parseFloat(
      (Math.max(open, close) * (1 + Math.random() * volatility * 0.5)).toFixed(
        config.decimals,
      ),
    );
    const low = parseFloat(
      (Math.min(open, close) * (1 - Math.random() * volatility * 0.5)).toFixed(
        config.decimals,
      ),
    );
