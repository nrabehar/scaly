...
export class AiService {
    ...
    private extractText(res: any): string | null {
        try {
            ...
            // Ollama /api/generate legacy shape
    ...
    private async tryProvider(
        ...
        try {
            ...
            if (!text) {
                this.logger.warn(
                    `provider ${name}: got response but could not extract text. Raw: ${JSON.stringify(raw).slice(0, 200)}`,
                );
                return { raw };
            }
            ...
            if (!parsed) {
                this.logger.warn(
                    `provider ${name}: could not parse JSON from: ${text.slice(0, 300)}`,
                );
                return { raw };
            }
            return { signal: parsed, raw };
        } catch (e) {
            this.logger.error(`provider ${name} threw: ${String(e)}`);
            return null;
        }
    }
        ...
        const providers = [
            {
                id: 'gemini',
                fn: () =>
                    this.gemini.call(prompt, this.config.get('GEMINI_API_KEY')),
            },
            {
                id: 'groq',
                fn: () =>
                    this.groq.call(prompt, this.config.get('GROQ_API_KEY')),
            },
            {
                id: 'openrouter',
                fn: () =>
