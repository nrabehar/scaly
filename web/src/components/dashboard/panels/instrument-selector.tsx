...
export function InstrumentSelector() {
	...
	const { status, isAnalyzing } = useSocket(currentSymbol);
	...
	return (
		<div className="flex items-center gap-3 flex-wrap">
			...
			<Select
				value={currentSymbol}
				onValueChange={(v) => setCurrentSymbol(v as SymbolType)}
			>
				<SelectTrigger className="w-35 h-8 text-sm font-semibold">
					<SelectValue />
				</SelectTrigger>
				<SelectContent>
			...
			<Select
				...
				onValueChange={(v) =>
					setCurrentTimeframe(v as TimeframeType)
				}
			>
